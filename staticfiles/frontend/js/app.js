...
async function handleFormSubmit(e, formType) {
  ...
  } else if (formType === 'academic_person') {
    phoneInput = form.querySelector('#phone_academic');
    itiInstance = window.itiAcademic;
  }
  if (!name1 || !name1.value || name1.value.trim().length < 3) {
    showError(name1, "Ism kamida 3 ta belgi.");
    return;
  }
  if (!name2 || !name2.value || name2.value.trim().length < 3) {
...
async function checkSession() {
  ...
  if (studentForm) {
    ...
    try {
      const resp = await fetch(`/api/v1/session-info/?session_id=${sid}`);
      const data = await resp.json();
      if (resp.ok && data.academy_persons && data.academy_persons.length > 0) {
        if (messageDiv) {
          messageDiv.textContent = `Xush kelibsiz, ${data.academy_persons[0].name1}!`;
          messageDiv.style.color = '#28a745';
        }
        fillAcademicForm(data.academy_persons[0]);
      }
    } catch (err) {
      if (messageDiv) {
        messageDiv.style.color = '#dc3545';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const academicForm = document.getElementById('academic-form');
  if (academicForm) {
    academicForm.addEventListener('submit', (e) => handleFormSubmit(e, 'academic_person'));
  }
